...
public interface ProducerBuilder<T> extends Cloneable {
     ...
     * 
     * 
     * This feature allows publisher to publish large size of message by splitting it to multiple chunks and let
     ...
     * So, it's better to reduce "maxPendingMessages" size to avoid producer occupying large amount of memory by buffered messages.
     * 4. Set message-ttl on the namespace to delete incomplete chunked messages. 
     * (sometime due to broker-restart or publish time, producer might fail to publish entire large message 
     * so, consumer will not be able to consume and ack those messages. So, those messages can be only discared by msg ttl) 
