...
public final class PageRangeCodec {
    ...
    /**
     * True if a page (one inclusive range per column) could contain a row satisfying
     * the predicate (AND across columns, OR within a column). Conservative: never
     * drops a page that could match. Pure byte comparison; schema-agnostic.
     */
    public static boolean pageCouldMatch(byte[] pageBlob, byte[] predicateBlob) {
        Map<Short, List<Range>> page = decode(pageBlob);
        Map<Short, List<Range>> pred = decode(predicateBlob);
        for (Map.Entry<Short, List<Range>> pc : pred.entrySet()) {
            List<Range> pageRanges = page.get(pc.getKey());
