...
class MessageCrypto {
   public:
     ...
     *
     */
    Result addPublicKeyCipher(std::set<std::string>& keyNames, const CryptoKeyReader& keyReader);
     ...
     */
    bool encrypt(std::set<std::string>& encKeys, const CryptoKeyReader& keyReader,
   ...
   private:
    ...
    bool getDigest(const std::string& keyName, const void* input, unsigned char keyDigest[],
                   unsigned int& digestLen);
    void removeExpiredDataKey();

    Result addPublicKeyCipher(const std::string& keyName, const CryptoKeyReader& keyReader);
