...
bool MessageCrypto::getDigest(const std::string& keyName, const void* input, unsigned char keyDigest[],
    ...
    digestLen = 0;
    if (EVP_DigestUpdate(mdCtx_, input, digestLen) != 1) {
        LOG_ERROR(logCtx_ + "Failed to get md5 hash for data key " + keyName);
        return false;
    }

    unsigned int mdLen = 0;
    if (EVP_DigestFinal_ex(mdCtx_, keyDigest, &mdLen) != 1) {
...
Result MessageCrypto::addPublicKeyCipher(std::set<std::string>& keyNames, const CryptoKeyReader& keyReader) {
    Lock lock(mutex_);
...
Result MessageCrypto::addPublicKeyCipher(const std::string& keyName, const CryptoKeyReader& keyReader) {
    if (keyName.empty()) {
...
bool MessageCrypto::encrypt(std::set<std::string>& encKeys, const CryptoKeyReader& keyReader,
...
bool MessageCrypto::decryptDataKey(const std::string& keyName, const std::string& encryptedDataKey,
                                   ...
                                   const CryptoKeyReader& keyReader) {
    StringMap keyMeta;
    ...
    if (!getDigest(keyName, dataKey_.get(), keyDigest, digestLen)) {
...
bool MessageCrypto::getKeyAndDecryptData(const proto::MessageMetadata& msgMetadata, SharedBuffer& payload,
    ...
    for (auto iter = msgMetadata.encryption_keys().begin(); iter != msgMetadata.encryption_keys().end();
        ...
        unsigned char keyDigest[EVP_MAX_MD_SIZE];
        unsigned int digestLen = 0;
        getDigest(keyName, encDataKey.c_str(), keyDigest, digestLen);
