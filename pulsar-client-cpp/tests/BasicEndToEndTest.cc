...
TEST(BasicEndToEndTest, testReceiveAsyncFailedConsumer)
{
    ClientConfiguration config;
    Client client(lookupUrl);
    std::string topicName = "persistent://prop/unit/ns1/closed-receiveAsync";
    std::string subName = "my-sub-name";
    ...
    int count = 0;
    // callback should immediately fail
    consumer.receiveAsync(boost::bind(&receiveCallBack, _1, _2, content, false, &isFailedOnConsumerClosed, &count));
...
{
    Client client(lookupUrl);
    std::string topicName = "persistent://prop/unit/ns/partition-closed-receive-async";
    ...
    // callback should immediately fail
    consumer.receiveAsync(
            boost::bind(&receiveCallBack, _1, _2, content, false, &isFailedOnConsumerClosing,
                        &closingCunt));
    // close consumer
    consumer.close();
