...
public class ExternReplicatorUtil {
    ...
    public static Response internalRegisterReplicatorOnTopic(PulsarService pulsar, TopicName topicName,
        ...
        FunctionConfig functionConfig = createFunctionConfig(pulsar, topicName, replicatorType, regionName);
        ...
        if (pulsar.getWorkerService() == null) {
            ...
            try {
                pulsar.getFunctionAdminClient().functions().createFunction(convert(functionConfig), replicatorFilePath);
        ...
        return internalRegisterReplicatorOnTopic(pulsar, topicName, replicatorType, regionName, functionConfig,
                replicatorFilePath);
    }
        ...
        try {
            ...
            internalUpdateReplicatorOnTopicAsync(pulsar, topicName, replicatorType, regionName, Action.Stop)
                    .thenAccept(stopResult -> {
                        ...
                        admin.persistentTopics().deleteSubscriptionAsync(topicName.toString(), subName)
                                .thenAccept(deleteRes -> {
                                    deleteResult.complete(null);
                                }).exceptionally(e -> {
                                    log.warn("[{}]-{} Failed to delete subscription {}, {}", topicName, replicatorType,
                                            subName, e.getMessage());
    ...
    private static Response internalRegisterReplicatorOnTopic(PulsarService pulsar, TopicName topicName,
            ReplicatorType replicatorType, String regionName, FunctionConfig functionConfig,
        ...
        return FunctionsImpl.registerFunction(CONF_REPLICATOR_TENANT_VAL, CONF_REPLICATOR_NAMESPACE_VAL,
                functionConfig.getName(), inputStream, fileDetail, functionConfig, pulsar.getWorkerService());
    }
    ...
    protected static void createNamespaceIfNotCreated(PulsarService pulsar, TopicName topic) {
        try {
            ...
            if (!pulsar.getConfigurationCache().propertiesCache().get(propertyPath).isPresent()) {
                ...
                propertyAdmin.setAdminRoles(pulsar.getConfiguration().getSuperUserRoles());
    ...
    public static org.apache.pulsar.functions.shaded.proto.Function.FunctionConfig convert(
            FunctionConfig functionConfig) throws IOException {
        org.apache.pulsar.functions.shaded.proto.Function.FunctionConfig.Builder functionConfigBuilder = org.apache.pulsar.functions.shaded.proto.Function.FunctionConfig
                .newBuilder();
