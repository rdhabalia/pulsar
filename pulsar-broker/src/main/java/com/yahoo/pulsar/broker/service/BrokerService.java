...
public class BrokerService implements Closeable, ZooKeeperCacheListener<Policies> {
    ...
    public void addUnAckedMessages(PersistentDispatcherMultipleConsumers dispatcher, int numberOfMessages) {
        ...
        if (unAckedMessages >= maxUnackedMessages
            ...
            // is blocked
            log.info("[{}] Blocking dispatcher due to reached max broker limit {}", dispatcher.getName(),
    ...
    public void unblockDispatchersOnUnAckMessages(List<PersistentDispatcherMultipleConsumers> iterator) {
        lock.writeLock().lock();
        try {
            iterator.forEach(dispatcher -> {
